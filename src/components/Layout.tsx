...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen font-sans text-[#1B1B1B] bg-white antialiased flex flex-col selection:bg-[#C9A961]/30 selection:text-[#0A0A0A]">
      <Header />
      <main id="main" className="pb-24 xl:pb-0 flex-1">
        <Outlet />
      </main>
      <Footer />

      {/* MOBILE STICKY CTA BAR — LOCKED, REPRODUCE EXACTLY */}
      <div className="fixed bottom-0 left-0 right-0 z-40 xl:hidden bg-white border-t border-[#E7E2D8] grid grid-cols-2 gap-2 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        ...
        <Link to={ctaTo} className="inline-flex items-center justify-center bg-[#C9A961] text-[#0A0A0A] py-3 rounded-lg text-[0.9375rem] font-semibold focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A961] focus-visible:ring-offset-2 focus-visible:ring-offset-white">
          {ctaLabel}
        </Link>
      </div>
    </div>
